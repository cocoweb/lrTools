...
import static com.foresee.test.loadrunner.lrapi4j.web.*;
...
public class web_Test {
  ...
  public void beforeTest() {
	  ...
	  HtmlUnitHelper.EnabledProxy("localhost", 8888);
  }
  ...
  public void beforeMethod(){
	  HtmlUnitHelper.resetHttpClient();
      
  }

  @Test
  public void submit_datatest() throws HttpException, lrapi.exceptions.HttpException, InterruptedException {
      url("tycx.do", 
  ...
  reg_save_param ("ret_cxTID", new String []{ 
          "NOTFOUND=ERROR", 
          "LB=<tid>", 
          "RB=</tid>" , 
          LAST} ); 
  reg_save_param ("ret_waitTime", new String []{ 
                  "NOTFOUND=ERROR", 
                  "LB=<waitingTime>", 
                  "RB=</waitingTime>" , 
                  LAST} ); 

  submit_data("tycx.do_3", 
  ...
  System.out.println(lr.eval_string("ret_waitTime:{ret_waitTime}"));
      ...
      do {
         // lr_x.think_time(lr_x.eval_int("{ret_waitTime}")/1000); 
          ...
          reg_find("Text=结果尚未返回", 
                  new String[]{ 
                  "SaveCount=ret_lunxunCount", 
                  LAST }); 

          reg_save_param ("retStr",
                          new String []{
                              "NOTFOUND=warning",
                              "LB=<cdmc>",
                              "RB=</cdmc>" ,
                              "Search=ALL",
                              LAST} );
          add_header ("Accept", "text/plain;charset=UTF-8" ); 

          reg_find("Text=<cdmc>综合查询</cdmc>", 
                          new String[]{ 
                          "SaveCount=ret_zhcxCount", 
                          LAST }); 

          submit_data("tycx.do_4",
                  "Action={p_tycxURL}/tycx/tycx/tycx.do", new String[]{
                  "Method=POST",
                  "RecContentType=text/plain",
                  "Referer={p_tycxURL}/tycx/tycx/tycx.do?action=toMenuList&menuId=SBXXCX_HZGS",
  ...
  public void setParameters() throws MalformedURLException {
      ...
      HtmlUnitHelper.setParameters(request, new String[]{
  ...
  @Test
  public void setHeaders() throws MalformedURLException {
    WebRequest request = new WebRequest(new URL("http://www.baidu.com")); 
	  
    HtmlUnitHelper.setHeaders(request, new String[]{ 
				"Resource=0", 
				"RecContentType=text/html", 
				"Referer=", 
				"Snapshot=t11.inf", 
				...
				"Url=../style/images-common/leftnav/right.gif", "Referer={p_tycxURL}/tycx/tycx/tycx.do?action=toTycxFrame&menuId=SBXXCX_HZGS&nsrsbh=441302730457424&nsrsmc=惠州市新大众眼镜有限公司&token=", ENDITEM, 
				LAST});
    Iterator<?> iter = request.getAdditionalHeaders().entrySet().iterator();
    while(iter.hasNext()){
