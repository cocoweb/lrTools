...
public class webhttpclient extends I_web {
    static Logger logger = Logger.getLogger(webhttpclient.class.getName());

    public static int url(String stepName, String urlAddress, String[] options) throws HttpException {
        HttpClientHelper.httpGet(StringUtil.parsarKVStrValue(eval_string(urlAddress)), options);
        return 0;
    }
    ...
    public static int submit_data(String stepName, String urlAddress, String[] options, String[] data)
        ...
        HttpClientHelper.httpPost(StringUtil.parsarKVStrValue(eval_string(urlAddress)), options, data);
    ...
    public static int custom_request(String stepName, String urlAddress, String[] optionsAndData) throws HttpException {
        if (optionsAndData[0].indexOf("Method=POST") == 0) {
            HttpClientHelper.httpPost(StringUtil.parsarKVStrValue(eval_string(urlAddress)), optionsAndData, new String[] {});
            // httprequest = new
            // HttpPost(StringUtil.parsarKVStrValue(eval_string(urlAddress)));
        } else if (optionsAndData[0].indexOf("Method=GET") == 0) {
            HttpClientHelper.httpGet(StringUtil.parsarKVStrValue(eval_string(urlAddress)), optionsAndData);
            // httprequest = new
    ...
    public static int add_header(String headerName, String headerValue) {
        HttpClientHelper.addHeader(headerName, headerValue);
        return 0;
    }

    public static int add_cookie(String cookieValue) {
        HttpClientHelper.addCookie(cookieValue.split(";"));
        return 0;
