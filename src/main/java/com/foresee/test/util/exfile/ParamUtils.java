...
public class ParamUtils {
    ...
    public static String eval_string(String paramString) {
        return  replaceVariables(paramString, ParameterCache.getInstance().getMapCache());
    }

    public static int save_string(String sValue, String ParaName) {
        ParameterCache.getInstance().putString(eval_string(ParaName), eval_string(sValue));
        return 0;
    }
	...
	private static String replaceVariables(String input, Map<String, String> variableMap, boolean useDefault,
		...
		while (matcher.find()) {
			String match = StringUtil.trimEnd(StringUtil.trimStart(matcher.group(1),EncodeUtils.decodeUnicode(left)),right);
