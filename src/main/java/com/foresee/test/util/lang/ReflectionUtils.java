...
//    }
    ...
    public static Object convertStringToObject(String value, Class<?> toType) {
        try {
            if (toType.isAssignableFrom(Boolean.class)) {
                logger.debug("boolean转化{}", value);
                if (StringUtils.isNotBlank(value) && value.equals("是"))
                    return true;
                if (StringUtils.isNotBlank(value) && value.equals("否"))
                    return false;
                return BooleanUtils.toBoolean(value);
            }
            if (toType.isAssignableFrom(Date.class)) {
                // 调用struts的转换处理
                return DateUtil.parseSimple(value);
                        //DateConverter.convertToDate(value);
            }
     ...
     *            D/S/B/ enum PropertyType
     * @param value
     * @return
     */
    public static Object convertStringToObject(String propertyTypeCode, String value) {
        Class<?> propertyType = Enum.valueOf(PropertyType.class, propertyTypeCode).getValue();
        Object propertyValue = convertStringToObject(value, propertyType);
